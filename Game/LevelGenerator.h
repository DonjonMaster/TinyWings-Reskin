...
private:
    void SpawnNextHill();
    // Changement ici : On déclare la fonction avec ses paramètres
    void SpawnCloudAt(float x, float targetY);
    void SpawnPlanet();
    ...
    const float Y_GROUND = -1700.0f;
    const float HILL_WIDTH = 1500.0f;
    const float HILL_SCALE = 4.0f;

    // --- PARAMÈTRES ÉTAGE 2 : LES NUAGES (2 COUCHES) ---
    float nextCloudX = 0.0f;
    const float Y_CLOUD_LAYER_1 = -2500.0f;
    const float Y_CLOUD_LAYER_2 = -3500.0f;
    const float CLOUD_WIDTH = 1500.0f;
    const float CLOUD_SCALE = 3.0f;

    // --- PARAMÈTRES ÉTAGE 3 : LES PLANÈTES ---
    float nextPlanetX = 0.0f;
    const float Y_PLANET_BASE = -5000.0f;
    const float PLANET_WIDTH = 1500.0f;
    ...
    const size_t MAX_CLOUDS = 14; // Doit être pair (7 colonnes de 2 nuages)
